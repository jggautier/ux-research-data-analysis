...
with
	all_versions_deaccessioned_or_draft as(
		...
		where
			dataset.id not in (
				select distinct datasetversion.dataset_id
				from datasetversion
				where versionstate in ('RELEASED')
			) and
	...
	dataset_depositors as(
		select
			...
			case
				when 
					authenticateduserlookup.authenticationproviderid = 'shib' and 
					authenticateduserlookup.persistentuserid like 'https://fed.huit.harvard.edu/idp/shibboleth%@harvard.edu'
					then 'shib-harvardkey'
				when 
					authenticateduserlookup.authenticationproviderid = 'shib'and 
					authenticateduserlookup.persistentuserid not like 'https://fed.huit.harvard.edu/idp/shibboleth%@harvard.edu' 
					then 'shib-other'
	...
	author_names as(
		select 
			...
			datasetfield1.parentdatasetfieldcompoundvalue_id, 
			datasetfieldvalue.value as author_name
	...
	author_affiliations as(
		select 
			datasetversion.dataset_id, 
			datasetfield1.parentdatasetfieldcompoundvalue_id, 
			datasetfieldvalue.value as author_affiliation
		from datasetfield datasetfield1
		join datasetfieldvalue on datasetfieldvalue.datasetfield_id = datasetfield1.id
		join datasetfieldcompoundvalue on datasetfieldcompoundvalue.id = datasetfield1.parentdatasetfieldcompoundvalue_id
		join datasetfield datasetfield2 on datasetfield2.id = datasetfieldcompoundvalue.parentdatasetfield_id
	...
	author_identifier_schemes as(
		select 
			datasetversion.dataset_id, 
			datasetfield1.parentdatasetfieldcompoundvalue_id, 
			controlledvocabularyvalue.strvalue as author_identifier_scheme
		from datasetfield datasetfield1
	...
	author_identifiers as(
		select 
			datasetversion.dataset_id, 
			datasetfield1.parentdatasetfieldcompoundvalue_id, 
			datasetfieldvalue.value as author_identifier
		from datasetfield datasetfield1
		join datasetfieldvalue on datasetfieldvalue.datasetfield_id = datasetfield1.id
		join datasetfieldcompoundvalue on datasetfieldcompoundvalue.id = datasetfield1.parentdatasetfieldcompoundvalue_id
		join datasetfield datasetfield2 on datasetfield2.id = datasetfieldcompoundvalue.parentdatasetfield_id
...
select
	...
	dataverse.id as dataverse_id, dataverse.alias as dataverse_alias
from dataset
